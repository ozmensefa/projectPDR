...
# Linux için --pool=solo kullanıyoruz (Windows uyumluluğu için)
echo "Celery worker başlatılıyor (pool=solo mode)..."
celery -A celery_worker.celery worker --loglevel=info --pool=solo -E

# Hata durumunda
if [ $? -ne 0 ]; then
    echo
    echo "[HATA] Celery worker başlatılamadı!"
    echo "Alternatif komut deneyin:"
    echo "  celery -A app.celery_config.celery worker --loglevel=info --pool=solo"
    exit 1
fi
